...
def spead_av(num=speednum,len=speedlen):
	sum,sumw = 0,0
	fib0,fib1,fib=0,1,1
...
def orderNow(buy):
	mycursor.execute("INSERT INTO `"+db_order+"` (`buy`,`price`,`id_init`,`status`,`price1`,`price2`,`id_fill`) VALUES (%s,%s,%s,%s,%s,%s,%s)"%(buy,price[i+1],id[i],3,price[i+1],price[i+1],id[i]))
	mydb.commit()
...
for i in range(speednum,len(id)-1):
	speed_AV = spead_av()
	if(speed_AV>35 and position == 0):
		orderNow(1)
		position = 1
		print(i, "1")
	elif(speed_AV<-35 and position == 1):
		orderNow(0)
		position = 0
		print(i , "0")
