...
mydb.commit()
mycursor.execute("INSERT INTO `"+db_order+"` (`buy`,`top`,`price`) VALUES (1,0,%s)"%(0))
mydb.commit()
#mycursor.execute("INSERT INTO `"+db_order+"` (`buy`,`top`,`price`) VALUES (1,1,%s)"%(0))
#mydb.commit()
#get all data

mycursor.execute("SELECT `id`,`pricemax` FROM `"+symbol+"` ORDER BY `id` ASC LIMIT "+str(long))
...
for i in range(speednum,len(id)-1):
	...
	order = myresult[0]
	if order[1] == 0: #sell 
		#if price[i] > order[2] or pos=="sell":
		pos = "sell"
		wait += 1
		if wait == 1 : 
			firsti = i
		elif (max(price[firsti:i]) - deltav) > price[i]:
			neworder(1)
			print ("sell",wait,price[i]-price[i-wait])
			pos = "non"
			wait=0
	else : #buy
		#if price[i] < order[2] or pos == "buy":
		pos="buy"
		wait += 1
		if wait == 1 : 
			firsti = i	
		elif ((min(price[firsti:i]) + deltav) < price[i]):
			neworder(0) 
